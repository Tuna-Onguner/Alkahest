    ...
    try {
      ...
      components.forEach((component: any) => {
        ...
        if (component.measures) {
          ...
          const duplicatedLinesMeasure = component.measures.find(
            (measure: any) => measure.metric === "duplicated_lines_density"
          );

          // Check if the measure exists and its value is greater than 0
          if (
            duplicatedLinesMeasure &&
            parseFloat(duplicatedLinesMeasure.value) > 0
          ) {
  ...
  public async getDuplications(
    ...
    try {
      ...
      for (const filePath of filePaths) {
        const response = await axios.get(
          `https://sonarcloud.io/api/duplications/show?key=${encodeURIComponent(
            filePath
          )}`,
          this._apiCallOptions
        );
